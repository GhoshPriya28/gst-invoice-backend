...
// 	return new Promise((resolve, reject) => {
// 		sequelize.query("SELECT a.product_id, a.quantity, a.selling_price, a.gst_percentage, a.taxable_price, a.subtotal_price,  b.product_unit, b.product_name, b.description FROM invoice_items as a inner join items as b on b.product_id = a.product_id where inv_id= '" + inv_id + "' AND a.is_deleted=0 AND a.quantity > 0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
// 			resolve(queryData)
// 		}).catch(error => {
...
//get details from invoice item table
exports.getitemDetailsByInvoiceId = async (inv_uni_id) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT a.product_id, a.quantity, a.selling_price, a.description, a.gst_percentage, a.taxable_price, a.subtotal_price,  b.product_unit, b.product_name FROM invoice_items as a inner join items as b on b.product_id = a.product_id where inv_uni_id= '" + inv_uni_id + "' AND a.quantity > 0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			resolve(queryData)
		}).catch(error => {
			console.log('Invoice Id Error', error)
			reject(error)
...
exports.getSumofSellingPriceByInvoiceId = async (inv_uni_id) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT SUM(subtotal_price) as totalSum FROM `invoice_items` WHERE inv_uni_id =  '" + inv_uni_id + "' and is_deleted=0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
...
exports.getSumofTaxablePriceByInvoiceId = async (inv_uni_id) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT SUM(taxable_price) as totalSum FROM `invoice_items` WHERE inv_uni_id =  '" + inv_uni_id + "' and is_deleted=0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			resolve(queryData[0].totalSum)
		}).catch(error => {
			console.log('Invoice Id Error', error)
			reject(error)
...
exports.date = async (callback=()=>{}) => {
	...
	console.log("lastId",parseInt(lastId));
	var lastid = 1
	if (lastId){
	   lastid = parseInt(lastId)+1
	}
	callback(currentYear + "-" + year,lastid)
}
...
exports.getUserDetailsByUserId = async (invoiceId) => {
	var userDetails = {}
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT * FROM users WHERE user_id = (SELECT user_id FROM invoices WHERE inv_uni_id = '" + invoiceId + "' and is_deleted=0)", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			if (queryData.length > 0) {
...
exports.getCustomerDetailsByInvoiceId = async (invoiceId) => {
	var customerDetails = {}
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT * FROM customers WHERE customer_id = (SELECT customer_id FROM invoices WHERE inv_uni_id = '" + invoiceId + "' and is_deleted=0)", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			if (queryData.length > 0) {
...
exports.getAddressDetailsByInvoiceId = async (invoiceId) => {
	var addressDetail = {}
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT * FROM address WHERE customer_id = (SELECT customer_id FROM invoices WHERE inv_uni_id = '" + invoiceId + "' and is_deleted=0)", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			if (queryData.length > 0) {
...
exports.getNeftDetailsByUserId = async (userId) => {
	return new Promise((resolve, reject) => {
		var neftDetails = {}
		sequelize.query("SELECT * FROM user_accounts WHERE user_id = '" + userId + "' and status=0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			console.log("queryDataneft",queryData);
			if (queryData.length > 0) {
				//neftDetails = queryData[0].bank_name + ' , ' + queryData[0].account_number + ' , ' + queryData[0].ifsc_code + ' , ' + queryData[0].account_holder
				// neftDetails = queryData[0]
				console.log("neftDetails",queryData);
				resolve(queryData)
			}
			else {
				resolve(neftDetails = '')
...
exports.getUserUpiDetailsByUserId = async (userId) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT * FROM user_upi WHERE user_id = '" + userId + "'", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
				resolve(queryData)
		}).catch(error => {
			console.log('UPI Details Error', error)
			reject(error)
		})
	})
}
...
exports.getProductHsnById = async (productId) => {
    return new Promise((resolve, reject) => {
        sequelize.query("SELECT item_code FROM items where product_id = '" + productId + "'", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
            if (queryData.length > 0) {
                let item_code = queryData[0].item_code;
                resolve(item_code)
            }
            else {
                resolve(item_code = '')
            ...
            console.log('Product item_code Error', error)
            reject(error)
        })
    })
}

exports.getProductTypeById = async (productId) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT type FROM items where product_id = '" + productId + "'", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
			if (queryData.length > 0) {
				let type = queryData[0].type;
				resolve(type)
			}
			else {
				resolve(type = '')
...
exports.getUserUpiDetailsByUserId = async (userId) => {
	return new Promise((resolve, reject) => {
		sequelize.query("SELECT * FROM user_upi WHERE user_id = '" + userId + "' AND status = 0", { type: sequelize.QueryTypes.SELECT }).then(queryData => {
				resolve(queryData)
		}).catch(error => {
			console.log('UPI Details Error', error)
			reject(error)
