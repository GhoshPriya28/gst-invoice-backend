            ...
            if (!errors.isEmpty()) {
                ...
                if (itemArray.length < 0) {
                    ...
                    await User.findOne({ where: { user_id: req.body.userId } }).then(async userDetails => {
                        if (userDetails) {
                            await Invoice.findOne({ where: { inv_id: invoiceId } }).then(async invoiceIdDetails => {
                                if (!invoiceIdDetails) {
                                    let totalPrice = 0;
                                ...
                                {
                                    customResponse(res, 1, "Given Invoice Id already exist in our system.")
                                }
                            }).catch(invoiceIdFindError => {
                                customResponse(res, 1, invoiceIdFindError)
                            })
                        }
                        else {
                            customResponse(res, 1, "Given User Id not exist in our system.")
                        }
                    }).catch(userIdFindError => {
                        customResponse(res, 1, userIdFindError)
                    })
                }
            }
        }
        ...
        await User.findOne({ where: { user_id: req.query.userId } }).then(async userDetails => {
            if (!userDetails) {
                ...
                if (invoiceDetails) {
                    ...
                    const userDetails = await queryHelper.getUserDetailsByUserId(req.query.inv_id);
                    const customerAddressDetails = await queryHelper.getAddressDetailsByInvoiceId(req.query.inv_id);
                    ...
                    let finalInvoiceDetails = {
                        ...
                        purchaseOrderDate: invoiceDetails.purchase_order,
                    ...
                    //finalInvoiceDetails.neftDetail = neftDetails
                    console.log('finalInvoiceDetails.taxArray', finalInvoiceDetails.taxArray)
                    ...
                    let options = { format: 'A4', path: invoicePath };
...
exports.uploadQrCode = [
    async (req, res) => {
        try {
            ...
            Invoice.findOne({ where: { inv_id: req.body.invId } }).then(invDetail => {
                if (invDetail) {
                    var qrDetails = {}
                    ...
                    Invoice.update(qrDetails, { where: { inv_id: req.body.invId } }).then(async updated => {
                        const qrData = await Invoice.findOne({ where: { inv_id: req.body.invId } });
                        if (qrData) {
        ...
        try {
            ...
            Invoice.findOne({ where: { inv_id: req.body.invId } }).then(invDetail => {
                if (invDetail) {
                    ...
                    Invoice.update(qrDetail, { where: { inv_id: req.body.invId } }).then(async updated => {
                        const qrData = await Invoice.findOne({ where: { inv_id: req.body.invId } });
                        if (qrData) {
                            const finalDetails = await getInvoiceResponseData(qrData)
        ...
        try {
            ...
            Invoice.findOne({ where: { inv_id: req.body.invId } }).then(invDetail => {
                if (invDetail) {
                    ...
                    Invoice.update(logoDetails, { where: { inv_id: req.body.invId } }).then(async updated => {
                        const logoData = await Invoice.findOne({ where: { inv_id: req.body.invId } });
                        if (logoData) {
