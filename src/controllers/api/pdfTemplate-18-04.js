...
.topSection{
  display:flex;
    ...
    .totalamountlist .total{
      text-align:right;
     
    }
    .totalamountlist .igst{
      text-align:right;
     
    }
    .totalamountlist .cgst{
      text-align:right;
     
    }
    .totalamountlist .sgst{
      text-align:right;
     
    }
    .totalamountlist .grandgsta{
      text-align:right;
     
    }

    totalamountlist .total span{
      
    }
    .totalamountlist .igst span{
      
    }
    .totalamountlist .cgst span{
      
    }
    .totalamountlist .sgst span{
      
    }
    .totalamountlist .grandgsta span{
...
<body class="body">
    ...
    <div id="logo" class="maindiv">
      ...
      <tr>
        ...
        <td width="40%" style="background:#eee;padding:20px;">
          <strong>Invoice No:</strong> ${finalInvoiceDetails.invoiceId ? finalInvoiceDetails.invoiceId : 'N/A'}<br>
          <strong>Invoice Date:</strong> ${finalInvoiceDetails.invoiceDate ? finalInvoiceDetails.invoiceDate : 'N/A'}<br>
  ...
  <main class="pdfmain">
    <table class="table">
      ...
      return ` 
        ...
        <tr class"totalamountlist">
          <td class="total" colspan="4" style="font-weight:bold;">SUB TOTAL: <br/> <span>${finalInvoiceDetails.invoiceTotal?.toFixed(2)}</span></td>
          <td class="igst" colspan="2" style="font-weight:bold;">${finalInvoiceDetails.invoiceGstType === 'igst' ? "IGST TOTAL: <br/> <span>" + finalInvoiceDetails.igstSum?.toFixed(2) + "</span>" : ''}</td>
          <td class="cgst" style="font-weight:bold;">${finalInvoiceDetails.invoiceGstType === 'cgst' || finalInvoiceDetails.invoiceGstType === 'sgst' ? "CGST TOTAL: <br/> <span>" + (finalInvoiceDetails.igstSum)?.toFixed(2) / 2 + "</span>" : ''}</td>
           <td class="sgst" style="font-weight:bold;">${finalInvoiceDetails.invoiceGstType === 'cgst' || finalInvoiceDetails.invoiceGstType === 'sgst' ? "SGST TOTAL: <br/> <span>" + (finalInvoiceDetails.igstSum)?.toFixed(2) / 2 + "</span>" : ''}</td>
          <td class="grandgsta" style="font-weight:bold;">GRAND TOTAL: <br/> <span>${finalInvoiceDetails.invoiceTotalAmounts?.toFixed(2)}</span></td>
        </tr>
      </tbody>
    </table>
  
    <p style="text-align: left">Amount Chargeable(in words)</p>
    <p style="text-align: left"><strong>Indian Rupees &nbsp;${finalInvoiceDetails.totalAmountInWords}&nbsp;Only</strong></p>
    ${neftDatas ? `
    ...
    ${finalInvoiceDetails.paymentQrCode ? ` <p style="text-align: left"><strong>Payment QR Code</strong></p>
  ...
  <div zcpage-footerhtml="true" style="clear: both;
  position: relative;
  height: 200px;
  bottom:0px">
    <p style="text-align:right">For &nbsp;${finalInvoiceDetails.companyName ? finalInvoiceDetails.companyName : 'N/A'}</p> <br />
    <div style="display:flex;  margin-top:100px; justify-content:space-between; align-items:center width:100%">
      <div style=" width:50%; margin:0;padding:0; display:flex; flex:1;" class="poweredBy">
    Powered By <img style='max-width:30px' src="${process.env.BASE_URL}assets/images/gstlogofooter.png" alt="logo" />
      </div> 
